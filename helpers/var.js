const { AutoYoutube } = require("../database/models"),
...
const findOrCreate = async (req, db, find) => {
    ...
    switch (db) {
        case 'AutoYoutube':
            try {
                if (req) {
                    let resp = await AutoYoutube.find(req);
                    return resp;
                }
            } catch (err) {
            ...
            break;
        case 'AutoYoutube-Find':
            try {
                if (req) {
                    let resp = await AutoYoutube.findOne({ guildID: req.guildID, ChannelName: req.ChannelName });
                    return resp;
                }
            } catch (err) {
                console.log(`FindOrCreate: AutoYoutube-Find has error: ${err.message}.`);
            }
            break;
        case 'AutoYoutube-Find2':
            try {
                if (req) {
                    let resp = await AutoYoutube.find(req);
                    return resp;
                ...
                console.log(`FindOrCreate: AutoYoutube-Find2 has error: ${err.message}.`);
            }
            break;
    }
}
